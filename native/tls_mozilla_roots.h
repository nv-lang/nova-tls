 ...
 *
 * Plan 195 F.1: bundled Mozilla CA root list for std/tls VerificationMode.SystemRoots.
 * AUTO-GENERATED (from git-for-windows curl ca-bundle.crt) -- do not hand-edit.
 *
 * Source: Mozilla CA certificate program root list (curl/git-for-windows
 * ca-bundle.crt) -- same lineage the previous rustls backend used via the Rust
 * webpki-roots crate: a static Mozilla bundle, NOT the OS certificate store.
 * VerificationMode.SystemRoots means "Mozilla bundled roots", matching the
 * pre-existing contract/tests.
 *
 * mbedtls_x509_crt_parse() PEM-parses this buffer at SystemRoots config time;
 * sizeof(...) includes the C strings trailing NUL, which mbedTLS requires
 * for PEM (vs DER) detection.
 */
#ifndef NOVA_TLS_MOZILLA_ROOTS_H
#define NOVA_TLS_MOZILLA_ROOTS_H

static const char NOVA_TLS_MOZILLA_ROOTS_PEM[] =
"-----BEGIN CERTIFICATE-----\n"
...
#endif /* NOVA_TLS_MOZILLA_ROOTS_H */
